...
bool parseClientRecord(const char *line, ClientDetails &client)
{
    ...
    client.minArea = atoi(fields[6].c_str());
    client.maxArea = atoi(fields[7].c_str());
    client.minPrice = (float)atof(fields[8].c_str());
    client.maxPrice = (float)atof(fields[9].c_str());
    client.sellerId = atoi(fields[10].c_str());
...
{
    Screen::beginRecord();
    Screen::printKeyValue("ID", to_string(client.id));
    ...
    Screen::printKeyValue("Area Range",
                          to_string(client.minArea) + " - " + to_string(client.maxArea));
    Screen::printKeyValue("Price Range",
                          formatFloat(client.minPrice) + " - " + formatFloat(client.maxPrice));
    Screen::printKeyValue("Pref Seller ID", to_string(client.sellerId));
    Screen::endRecord();
}
...
{
    int id = 0;
    FILE *file = fopen("clientID.txt", "r");
...
void Client::updateClientRequirements(int id)
{
    ...
    while (readClientRecord(original, client))
    {
        ...
        {
            found = true;
            int previousSellerId = client.sellerId;

            string location = Input::readString(
                string("Location (Current: ") + client.location + ", blank/0 for any): ",
                client.location);
            Input::copyTo(client.location, sizeof(client.location), location);

            client.minArea = Input::readInt(
                string("Minimum Area (Current: ") + to_string(client.minArea) + "): ",
                client.minArea);
            client.maxArea = Input::readInt(
                string("Maximum Area (Current: ") + to_string(client.maxArea) + "): ",
                client.maxArea);
            client.minPrice = Input::readFloat(
                string("Minimum Price (Current: ") + formatFloat(client.minPrice) + "): ",
                client.minPrice);
            client.maxPrice = Input::readFloat(
                string("Maximum Price (Current: ") + formatFloat(client.maxPrice) + "): ",
                client.maxPrice);
            client.sellerId = Input::readInt(
                string("Preferred Seller ID (Current: ") + to_string(client.sellerId) + ", 0 for any): ",
                client.sellerId);
    ...
    while (readClientRecord(original, client))
    {
        ...
        if (client.id == id)
        {
            ...
            string newName = Input::readString(
                string("Enter New Name (Current: ") + client.name + "): ",
                client.name);
            Input::copyTo(client.name, sizeof(client.name), newName);

            string newAddress = Input::readString(
                string("Enter New Address (Current: ") + client.address + "): ",
                client.address);
            Input::copyTo(client.address, sizeof(client.address), newAddress);

            string newPhone = Input::readString(
                string("Enter New Phone (Current: ") + client.phone + "): ",
                client.phone);
            Input::copyTo(client.phone, sizeof(client.phone), newPhone);

            string newEmail = Input::readString(
                string("Enter New Email (Current: ") + client.email + "): ",
                client.email);
...
void Client::viewClients()
{
    ...
    do
    {
        ...
        if (choice == 1)
        {
            int id = Input::readInt("Enter Client ID to delete: ");
            deleteClientById(id);
        }
        else if (choice == 2)
        {
            int id = Input::readInt("Enter Client ID to update: ");
            updateClientById(id);
        }
        // else if (choice == 3)
        // {
